...
void CVideoSyncFb::Run(CEvent& stopEvent)
{
  ...
  while (!stopEvent.Signaled() && !m_abort)
  {
    ...
    if(ioctl(m_fd_fb, FBIO_WAITFORVSYNC) < 0)
      CLog::Log(LOGERROR, "CVideoReferenceClock: ioctl FBIO_WAITFORVSYNC errno={}", errno);

    gettimeofday(&curr, NULL);
    timeval_subtract(&diff, &curr, &last);
    diff.tv_usec += (1000000 / fps / 2);
    uint64_t now = CurrentHostCounter();
    m_refClock->UpdateClock(diff.tv_usec / (1000000 / fps), now);
    last = curr;
  }
}
...
{
  CLog::Log(LOGDEBUG, "CVideoReferenceClock: cleaning up Fb{}", m_fbNum);
  if(m_fd_fb >= 0)
     close(m_fd_fb);
