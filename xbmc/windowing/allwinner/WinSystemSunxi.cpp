  ...
  m_libinput(new CLibInputHandler)
{
  ...
  if (fd >= 0)
  {
     CLog::Log(LOGINFO, "{} - graphics sysfs is writable\n", __FUNCTION__);
     m_readonly = false;
  }
  else
  {
     CLog::Log(LOGINFO, "{} - graphics sysfs is read-only\n", __FUNCTION__);
     m_readonly = true;
  }
  close(fd);
  ...
  if (fd < 0)
  {
     CLog::Log(LOGERROR, "{} - Error while opening /dev/fb0.\n", __FUNCTION__);
     return;
  }
  // Unblank the fb
  if (ioctl(fd, FBIOBLANK, 0) < 0)
  {
     CLog::Log(LOGERROR, "{} - Error while unblanking fb0.\n", __FUNCTION__);
  }
  close(fd);
  VLInit();
}
...
bool CWinSystemSunxi::InitWindowSystem()
{
  ...
  VIDEOPLAYER::CProcessInfoSunxi::Register();
                                    ...
                                    RESOLUTION_INFO& res)
{
  ...
  std::chrono::milliseconds delay(
      CServiceBroker::GetSettingsComponent()->GetSettings()->GetInt("videoscreen.delayrefreshchange") * 100);
...
bool CWinSystemSunxi::SetDisplayResolution(const RESOLUTION_INFO &res, std::string framebuffer_name)
{
  ...
  if(fbMode.Exists())
      cur_mode = fbMode.Get<std::string>().value_or("");
  CLog::Log(LOGINFO, "CWinSystemSunxi::{}: current resolution '{}'",__FUNCTION__, cur_mode);
  ...
  CLog::Log(LOGINFO, "CWinSystemSunxi::{}: changing to resolution '{}'",__FUNCTION__, mode);
...
void CWinSystemSunxi::SetFramebufferResolution(int width, int height, std::string framebuffer_name)
{
  ...
  if ((fd0 = open(framebuffer.c_str(), O_RDWR)) >= 0)
  {
    ...
    {
      CLog::Log(LOGINFO, "CWinSystemSunxi::{}: FBIOGET_VSCREENINFO to width:{}, height:{}",__FUNCTION__, width, height);
      vinfo.xres = width;
      vinfo.yres = height;
      ...
      if(ioctl(fd0, FBIOPUT_VSCREENINFO, &vinfo)<0)
        CLog::Log(LOGWARNING, "CWinSystemSunxi::{}: FBIOSET_VSCREENINFO failed",__FUNCTION__);
    }
    close(fd0);
  }
}
...
void CWinSystemSunxi::UpdateResolutions()
{
  ...
  for (size_t i = 0; i < resolutions.size(); i++)
  {
    ...
    CLog::Log(LOGINFO, "Found resolution {} x {} with {} x {}{} @ {:f} Hz",
  ...
  {
    CLog::Log(LOGINFO, "Found ({}x{}{}@{:f}) at {}, setting to RES_DESKTOP at {}",
...
bool CWinSystemSunxi::GetNativeResolution(RESOLUTION_INFO *res)
{
  ...
  CLog::Log(LOGINFO,"Current resolution: {}\n",res->strMode.c_str());
  return true;
}
  ...
  for (size_t i = 0; i < probe_str.size(); i++)
  {
     if(!StringUtils::StartsWith(probe_str[i], "D:") && !StringUtils::StartsWith(probe_str[i], "U:") &&
        !StringUtils::StartsWith(probe_str[i], "V:"))
         continue;
     CLog::Log(LOGINFO, "Processing mode string {}\n", probe_str[i]); 
...
bool CWinSystemSunxi::VLInit()
{
  ...
  if (m_hdisp == -1)
  {
    CLog::Log(LOGERROR, "A10: open /dev/disp failed. ({})", errno);
    return false;
  } 
  ...
  {
  CLog::Log(LOGERROR, "A10: version failed. ({})", errno);
  return false;
}
#endif
