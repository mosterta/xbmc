...
bool CHwLayerAllwinnerDisp2::calcCroppedValues(cdRect_t *src, cdRect_t *scn, unsigned int screen_width)
{
  if (scn->y < 0)
  {
    int scn_clip = -scn->y;
...
{
  CLog::Log(LOGDEBUG, " (CHwLayerAllwinnerDisp2) {}", __FUNCTION__);
  bool status = true;
  ...
  {
    CLog::Log(LOGERROR, "CHwLayerAllwinnerDisp2:{} Failure in getting frame configuration!", __FUNCTION__);
    return false;
  }
...
bool CHwLayerAllwinnerDisp2::getSyncFenceValue(int fence, HwLayerSyncValue &value)
{
  ...
  {
     CLog::Log(LOGERROR, "CHwLayerAllwinnerDisp2: syncFrame: returned error:{} errno:{} fence:{}\n", status, errno, fence);
  }
  return retval;
}
