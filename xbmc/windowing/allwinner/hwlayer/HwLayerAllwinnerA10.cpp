...
bool CHwLayerAllwinnerA10::create(HwLayerType type)
{
  ...
  if(type == HwLayerType::GUI)
  {
    ...
    {
      CLog::Log(LOGERROR, "CHwLayerAllwinnerA10: get fb0 layer handle failed. ({})", errno);
      status = false;
    }
    else
      m_layerCreated = true;
...
bool CHwLayerAllwinnerA10::hide()
{
  ...
  if(m_layerVideoStarted)
  {
    ...
    if(error < 0)
    {
      CLog::Log(LOGERROR, "CHwLayerAllwinnerA10: video start failed, errno={}\n", errno);
      status = false;
    }
  ...
  if(m_layerOpened)
  {
    ...
    if(error < 0)
    {
      CLog::Log(LOGERROR, "CHwLayerAllwinnerA10: layer open failed, errno={}\n", errno);
      status = false;
    }
    m_layerOpened = false;
  }
...
bool CHwLayerAllwinnerA10::getSyncFenceValue(int fence, HwLayerSyncValue &value)
{
  ...
  if(m_layerVideoStarted)
  {
    ...
    if(! status )
      CLog::Log(LOGERROR, "CHwLayerAllwinnerA10:{} error calling getCurrentFrameId", __FUNCTION__);
...
bool CHwLayerAllwinnerA10::getCurrentFrameId(int &frameId)
{
  ...
  if(frameId < 0)
  {
    CLog::Log(LOGERROR, "get frame id failed m_dispFd={} frameId={}\n", m_config.m_dispFd, frameId);
    status = false;
  }
  return status;
};
...
bool CHwLayerAllwinnerA10::setAlphaValue(int alphaValue)
{
  ...
  {
    CLog::Log(LOGERROR, "CHwLayerAllwinnerA10: DISP_CMD_LAYER_ALPHA_VALUE for layer({}) failed.",
           m_layerId);
    return false;
  }
  return true;
...
bool CHwLayerAllwinnerA10::setAlphaEnable(int alphaEnable)
{
  ...
  if (ioctl(m_config.m_dispFd, cmd, &args) < 0)
  {
    CLog::Log(LOGERROR, "CHwLayerAllwinnerA10: DISP_CMD_LAYER_ALPHA_ON/OFF for layer({}) failed.",
         m_layerId);
    return false;
  }
  return true;
...
bool CHwLayerAllwinnerA10::setColorKeyEnable(int ckEnable)
{
  ...
  if (ioctl(m_config.m_dispFd, cmd, &args) < 0)
  {
    CLog::Log(LOGERROR, "CHwLayerAllwinnerA10: DISP_CMD_LAYER_CK_ON/OFF for layer({}) failed.",
           m_layerId);
    return false;
  }
  return true;
...
bool CHwLayerAllwinnerA10::calcCroppedValues(cdRect_t *src, cdRect_t *scn, unsigned int screen_width)
{
  if (scn->y < 0)
  {
    int scn_clip = -scn->y;
