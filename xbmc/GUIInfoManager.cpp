...
int CGUIInfoManager::TranslateSingleString(const std::string &strCondition, bool &listItemDependent)
{
  ...
  else if (info.size() == 2)
  {
    ...
    if (cat.name == "integer")
    {
      ...
      {
        int value = -1;
        char *endPtr = (char*)(prop.param(0).data() + prop.param(0).size());
        value = strtol(prop.param(0).data(), &endPtr, 10);
          ...
          auto getIntValue = [this, &item, &contextWindow](int infoNum) {
            ...
            if (!GetInt(intValue, infoNum, contextWindow, item))
            {
              ...
              if (value.find_first_of(':') != value.npos)
                ...
                char *endPtr = value.data() + value.size();
                intValue = strtol(value.data(), &endPtr, 10);
              }
            }
            return intValue;
          };
