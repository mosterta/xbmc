...
{
  CLog::Log(LOGDEBUG, " (CVideoBufferRefSunxi) {}, surf:{}", __FUNCTION__, m_surf);
...
void CVideoBufferRefSunxi::config(AVCodecContext *avctx, int chromaType, int ycbcrFormat, int width, int height)
{
  if(m_chromaType != chromaType || m_ycbcrFormat != ycbcrFormat || m_width != width || m_height != height)
  {
    ...
    if(m_surf != VDP_INVALID_HANDLE)
    {
      CLog::Log(LOGDEBUG, "CVideoBufferRefSunxi::{} - (re-)config - Video surface={} destroyed", __FUNCTION__, m_surf);
      m_interop.glVDPAUDestroySurfaceCedar(m_surf);
    }
    ...
    if (vdp_st != 0)
    {
      CLog::Log(LOGERROR, "CVideoBufferRefSunxi::{} - No Video surface available could be created", __FUNCTION__);
      return ;
    }
    CLog::Log(LOGDEBUG, " (CVideoBufferRefSunxi) {} created new surface {}", __FUNCTION__, m_surf);
  } 
}

void CVideoBufferRefSunxi::createBuffer()
{
  CLog::Log(LOGDEBUG, "CVideoBufferRefSunxi::{} - create buffer ref surface, surface:{}", __FUNCTION__, m_surf);
  if(!m_bufRef)
...
{
  CLog::Log(LOGDEBUG, " (CVideoBufferRefPoolSunxi) {}", __FUNCTION__ );
...
CVideoBufferRefSunxi* CVideoBufferRefPoolSunxi::Get(AVCodecContext *avctx, int chromaType, int ycbcrFormat, int width, int height, int format)
{
  ...
  if (!m_free.empty())
  {
    ...
    CLog::Log(LOGDEBUG, " (CVideoBufferRefPoolSunxi) {} re-use buffer {}", __FUNCTION__, idx);
  }
  if (buf == NULL)
  {
    int id = m_all.size();
    ...
    m_all.push_back(buf);
    m_used.push_back(id);
    CLog::Log(LOGDEBUG, " (CVideoBufferRefPoolSunxi) {} create new buffer {}", __FUNCTION__, id);
  }
...
{
  CLog::Log(LOGDEBUG, " (CVideoBufferRefPoolSunxi) {}, buffer id:{}", __FUNCTION__, id);
  std::unique_lock<CCriticalSection> lock(m_critSection);
...
{
  CLog::Log(LOGDEBUG, " (CVideoBufferPoolSunxi) {}", __FUNCTION__ );
  std::unique_lock<CCriticalSection> lock(m_critSection);
  for (auto buf : m_all)
  {
    delete buf;
...
CVideoBuffer* CVideoBufferPoolSunxi::Get()
{
  ...
  if (!m_free.empty())
  {
    ...
    m_used.push_back(idx);
    buf = m_all[idx];
    CLog::Log(LOGDEBUG, " (CVideoBufferPoolSunxi) {}, re-use buffer {}", __FUNCTION__ ,idx);
  }
  else
  {
    int id = m_all.size();
    ...
    m_all.push_back(buf);
    m_used.push_back(id);
    CLog::Log(LOGDEBUG, " (CVideoBufferPoolSunxi) {} new buffer {}", __FUNCTION__, id);
  }
...
{
  CLog::Log(LOGDEBUG, " (CVideoBufferPoolSunxi) {}, buffer id:{}", __FUNCTION__, id);
  std::unique_lock<CCriticalSection> lock(m_critSection);
...
int CVideoBufferPoolSunxi::FFGetBuffer(AVCodecContext *avctx, AVFrame *pic, int flags)
{
  ...
  switch (pic->format) {
    ...
    case AV_PIX_FMT_YUV444P: 
      ...
      break;
    default:
      CLog::Log(LOGERROR, " (FFMPEG) {}: Unsupported data layout: {}", __FUNCTION__,
...
void CVideoBufferRefSunxi::FFReleaseBuffer(void *opaque, uint8_t *data)
{
  ...
  CLog::Log(LOGDEBUG, " (CVideoBufferRefSunxi) {} releasing videobuf id={}", __FUNCTION__, bufRef->GetId());
}
