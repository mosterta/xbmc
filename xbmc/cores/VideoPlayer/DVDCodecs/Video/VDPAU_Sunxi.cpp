...
void CVDPAUContext::SpewHardwareAvailable()  //Copyright (c) 2008 Wladimir J. van der Laan  -- VDPInfo
{
  ...
  for(const CDecoder::Desc& decoder_profile : decoder_profiles)
  {
    ...
    {
      CLog::Log(LOGINFO,"{:>16s} {:2d} {:5d} {:5d} {:5d}", decoder_profile.name,
...
bool CDecoder::Open(AVCodecContext* avctx, AVCodecContext* mainctx, const enum AVPixelFormat fmt)
{
...

  {
    ...
    if(found)
    {
      ...
      {
        CLog::Log(LOGERROR, "VDPAU::Open: error {}({}) checking for decoder support", m_vdpauConfig.context->GetProcs().vdp_get_error_string(vdp_st), vdp_st);
        return false;
      }
      ...
      {
        CLog::Log(LOGWARNING,"VDPAU::Open: requested picture dimensions ({}, {}) exceed hardware capabilities ( {}, {}).",
	                      avctx->coded_width, avctx->coded_height, max_width, max_height);
        return false;
      }

      if (!CDVDCodecUtils::IsVP3CompatibleWidth(avctx->coded_width))
        CLog::Log(LOGWARNING,"VDPAU::Open width {} might not be supported because of hardware bug", avctx->width);
...
void CDecoder::SetWidthHeight(int width, int height)
{
  ...
  CLog::Log(LOGDEBUG, LOGVIDEO, "CVDPAUSunxi::SetWidthHeight Setting OutWidth: {} OutHeight: {}", m_vdpauConfig.outWidth, m_vdpauConfig.outHeight);
}
...
CDVDVideoCodec::VCReturn CDecoder::Check(AVCodecContext* avctx)
{
  ...
  if (state == VDPAU_LOST)
  {
    ...
    auto duration = std::chrono::milliseconds(4000);
    if (!m_DisplayEvent.Wait(duration))
    {
...
bool CDecoder::ConfigVDPAU(AVCodecContext* avctx, int ref_frames)
{
  ...
  CLog::Log(LOGINFO, " (VDPAU) screenWidth:{} vidWidth:{} surfaceWidth:{}",m_vdpauConfig.outWidth,m_vdpauConfig.vidWidth,m_vdpauConfig.surfaceWidth);
  CLog::Log(LOGINFO, " (VDPAU) screenHeight:{} vidHeight:{} surfaceHeight:{}",m_vdpauConfig.outHeight,m_vdpauConfig.vidHeight,m_vdpauConfig.surfaceHeight);
  ...
  if (CheckStatus(vdp_st, __LINE__))
    return false;
  if(adaptedSurfaceWidth != (uint32_t)m_vdpauConfig.surfaceWidth || adaptedSurfaceHeight != (uint32_t)m_vdpauConfig.surfaceHeight)
  {
    CLog::Log(LOGINFO, " (VDPAU) adapted surfaceWidth:{}",adaptedSurfaceWidth);
    CLog::Log(LOGINFO, " (VDPAU) adapted surfaceHeight:{}",adaptedSurfaceHeight);
                     ...
                     const VdpBitstreamBuffer *buffers)
{
  ...
  CLog::Log(LOGDEBUG, " (VDPAUSunxi)::{} render surface:{}",__FUNCTION__, surf);
  ...
  if (diff*1000/CurrentHostFrequency() > 30)
    CLog::Log(LOGDEBUG, LOGVIDEO, "CVDPAU::DrawSlice - VdpDecoderRender long decoding: {} ms, dec: {}, proc: {}, rend: {}", (int)((diff*1000)/CurrentHostFrequency()), decoded, processed, rend);
...
IHardwareDecoder* CDecoder::Create(CDVDStreamInfo &hint, CProcessInfo &processInfo, AVPixelFormat fmt)
 {
   if (CDecoder::IsVDPAUFormat(fmt) && CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_VIDEOPLAYER_USEVDPAU))
     return new VDPAUSunxi::CDecoder(*VIDEOPLAYER::CProcessInfoSunxi::Create());
