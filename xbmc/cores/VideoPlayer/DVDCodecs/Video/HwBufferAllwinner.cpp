...
{
  CLog::Log(LOGNOTICE, " (VDPAU) {}", __FUNCTION__);
...
int CCedarRender::video_get_buffer(AVCodecContext *s, AVFrame *pic)
{
    ...
    if (!desc) {
        CLog::Log(LOGERROR,
            "Unable to get pixel format descriptor for format {}\n",
    ...
    vdp->m_vdpProcs.glVDPAUGetMappedMemoryCedar(
				      ...
				      &buf[1], &buf[2]);
#if VDPAU_DEBUG
    CLog::Log(LOGERROR, "CVDPAU::{} - (re-)used video surface={}", __FUNCTION__, pic->buf[0]->data);
#endif
...
int CCedarRender::update_frame_pool(AVCodecContext *avctx, AVFrame *frame)
{
	...
	case AV_PIX_FMT_YUV444P: 
	  ...
	  break;
	default:
	  CLog::Log(LOGERROR, " (VDPAU) {}: Unsupported data layout: {}", __FUNCTION__,
...
AVBufferRef *CCedarRender::pool_alloc(void *opaque, int size)
{
    ...
    {
#if VDPAU_DEBUG
       CLog::Log(LOGERROR, "CVDPAU::FFGetBuffer - Video surface={} created", surf);
#endif
    }
...
#if VDPAU_DEBUG
  CLog::Log(LOGERROR, " (VDPAU) {} releasing video surface={}", __FUNCTION__, surf);
#endif
}
...
bool CCedarRender::GetPicture(AVCodecContext* avctx, AVFrame* frame, DVDVideoPicture* picture)
{
  ...
  if(picture->cedarRender)
  {
    ...
    additionalData->Sync();
#if VDPAU_DEBUG
      CLog::Log(LOGERROR, " (VDPAU) {} GetPicture video surface={}", __FUNCTION__, picture->cedarRender-> buf[0]->data);
#endif
...
bool CCedarRender::AcquireBuffer(void *frame)
{
    ...
    if(pic)
    {
      ...
      pic->added = true;
#if VDPAU_DEBUG
      CLog::Log(LOGERROR, " (VDPAU) {} mapped video surface={} to surface {}", __FUNCTION__, surf, glVdpauSurfaceCedar);
#endif
    }
    else
    {
      	CLog::Log(LOGERROR, " (VDPAU) {} pic==NULL", __FUNCTION__);
    }
...
#if VDPAU_DEBUG
	CLog::Log(LOGERROR, " (VDPAU) {} unmapped surface={}", __FUNCTION__, pic->surfaceCedar);
#endif	
      }
      cedar = pic->vdp;
      delete pic;
...
bool CCedarRender::CheckStatus(VdpStatus vdp_st, int line)
{
  if (vdp_st != VDP_STATUS_OK)
  {
    CLog::Log(LOGERROR, " (VDPAU) CCedarRender Error: ({}) at {}:{}\n", vdp_st, __FILE__, line);
...
//  glVDPAUGetSurfaceivCedar = NULL;
  ...
  if(!m_vdpProcs.m_dlVdpauNvHandle)
  {
    ...
    snprintf(driver_path, 255, "{}/libcedarDisplay.so.1", VDPAU_MODULE_DIR);
