...
CRendererVDPAUSunxi::CRendererVDPAUSunxi()
{
  ...
  {
    CLog::Log(LOGERROR, "CRendererVDPAUSunxi:{} initialize failed", __FUNCTION__);
  }
...
CRendererVDPAUSunxi::~CRendererVDPAUSunxi()
{
  ...
  CLog::Log(LOGDEBUG, "CRendererVDPAUSunxi:{} end", __FUNCTION__);
}
...
bool CRendererVDPAUSunxi::Configure(const VideoPicture& picture, float fps, unsigned int orientation)
{
  ...
  if(! status )
    CLog::Log(LOGERROR, "CRendererVDPAUSunxi:{} error setting property scaler", __FUNCTION__);
  ...
  if(! status )
    CLog::Log(LOGERROR, "CRendererVDPAUSunxi:{} error calling showlayer", __FUNCTION__);
...
{
  CLog::Log(LOGDEBUG, "CRendererVDPAUSunxi:{} saveBuffers={}", __FUNCTION__, saveBuffers);
...
void CRendererVDPAUSunxi::RenderUpdate(int index, int index2, bool clear, unsigned int flags, unsigned int alpha)
{
  ...
  if (m_iLastRenderBuffer == index)
  {
    ...
    KODI::TIME::Sleep(std::chrono::milliseconds(10));
...
bool CRendererVDPAUSunxi::RenderCapture(int index, CRenderCapture* capture)
{
  capture->BeginRender();
  capture->EndRender();
  return false;
...
bool CRendererVDPAUSunxi::Supports(ERENDERFEATURE feature) const
{
...
bool CRendererVDPAUSunxi::Supports(ESCALINGMETHOD method) const
{
    return false;
}
